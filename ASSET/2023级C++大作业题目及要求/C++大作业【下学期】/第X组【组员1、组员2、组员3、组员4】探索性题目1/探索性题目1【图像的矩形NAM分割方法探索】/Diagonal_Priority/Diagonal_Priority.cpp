...
bool isSameBlock(Mat f, Mat mark, Point lt, Point rb, int xgm) {
    ...
    if (lt.x < rb.x && lt.y < rb.y) {
        ...
        for (int j = lt.y; j <= rb.y; j++) {
            for (int i = lt.x; i <= rb.x; i++) {
                ...
                // cout << g1 << " " << g2 << " " << g3 << " " << g4 << " " <<
                // g5 << " " << g6 << " " << gest << "
                // "<<f.at<uchar>(Point(i,j))<<endl;
                if (abs(f.at<uchar>(Point(i, j)) - gest) > xgm ||
                    mark.at<uchar>(Point(i, j)) == 1) {
                    flag = 0;
                    break;
                }
            }
    ...
    // 宽为1的矩形
        ...
        for (int i = lt.x; i <= rb.x; i++) {
            ...
            if (abs(f.at<uchar>(Point(i, lt.y)) - gest) > xgm ||
                mark.at<uchar>(Point(i, lt.y)) == 1) {
                flag = 0;
                break;
            }
        }
    ...
    // 长为1的矩形
        ...
        for (int j = lt.y; j <= rb.y; j++) {
            ...
            if (abs(f.at<uchar>(Point(lt.x, j)) - gest) > xgm ||
                mark.at<uchar>(Point(lt.x, j)) == 1) {
                flag = 0;
                break;
            }
        }
...
Point REHelper(Mat f, Mat mark, int M, int N, Point start, int xgm) {
    Point end = start;
    Point end1, end2;
    while (end.x < M && end.y < N && (mark.at<uchar>(end) == 0) &&
           (mark.at<uchar>(start) == 0)) {
        if (!isSameBlock(f, mark, start, end, xgm))
            break;
        end.x++;
        end.y++;
    }
    ...
    while (end1.x < M && (mark.at<uchar>(end1) == 0)) {
        if (!isSameBlock(f, mark, start, end1, xgm))
            break;
        end1.x++;
    }
    end1.x--;
    ...
    while (end2.y < N && (mark.at<uchar>(end2) == 0)) {
        if (!isSameBlock(f, mark, start, end2, xgm))
            break;
        end2.y++;
    }
    end2.y--;

    if ((end2.x - start.x + 1) * (end2.y - start.y + 1) >
        (end1.x - start.x + 1) * (end1.y - start.y + 1))
        return end2;
    else
        return end1;
    ...
    Point end;
    if
    ((end_first.x-start.x+1)*(end_first.y-start.y+1)<=(end_second.x-start.x+1)*(end_second.y-start.y+1))end
    = end_second; else end = end_first; if
    ((end.x-start.x+1)*(end.y-start.y+1)<(end_third.x-start.x+1)*(end_third.y-start.y+1))end
    = end_third;*/
}

// 灰度图f编码
void RNAMCEncoding(Mat f, Mat &R, Mat &markMatrix, int M, int N,
                   vector<Color> &p, vector<char> &q, int xgm) {
    Point start, end;
    for (int i = 0; i < M; i++)
        for (int j = 0; j < N; j++) {
...
double BPP(vector<Color> p, int M, int N, vector<char> q) {
    int a1 = 0, a2 = 0, a3 = 0;
    for (int i = 0; i < p.size(); i++) {
        if (p[i].getGrey()[0] != -1 && p[i].getGrey()[3] != -1)
            a1++;
        else if (p[i].getGrey()[1] == -1 && p[i].getGrey()[2] == -1)
            a3++;
        else
            a2++;
    }
...
////实现往坐标表里插入位置的功能
// void CEHelper(vector<bool>& b, int cur_pos, int last_pos = -1, int last_value
// = 0) {
//	//用来判断需要多少个bit位
//	int d;
//	//记录序号或者距离
...
//
////实现从坐标表里取出位置的功能
// int CDHelper(vector<bool>& b, vector<bool>::iterator& i, int last_pos = -1,
// int last_value = 0) {
//	//用来判断需要多少个bit位
//	int d;
//	if (last_pos != -1)
